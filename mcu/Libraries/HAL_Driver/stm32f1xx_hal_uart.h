...
#include "stm32f1xx_hal_dma.h"
  ...
  typedef struct
  {
    ...
    uint32_t OverSampling; /*!< Specifies whether the Over sampling 8 is enabled
                              ...
                              UART_Over_Sampling. This feature is only available
                                on STM32F100xx family, so OverSampling parameter
                              should always be set to 16. */
  } UART_InitTypeDef;
   ...
   * done (Peripheral initialized. HAL UART Init function already called) b4-b3
   * (not used) xx : Should be set to 00 b2     Intrinsic process state 0  :
   * Ready 1  : Busy (Peripheral busy with some configuration or internal
   * operations) b1     (not used) x  : Should be set to 0 b0     Tx state 0  :
   ...
   * has occurred leading to call of the RxEvent callback. This parameter can be
   * a value of @ref UART_RxEvent_Type_Values : HAL_UART_RXEVENT_TC = 0x00U,
