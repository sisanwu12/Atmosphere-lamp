...
| 中断与健壮性   | `mcu/user/stm32f1xx_it.c` `mcu/user/freertos_hooks.c` | 异常/中断          | ISR 转发与故障陷入      | 保证系统稳定运行     |
| BLE 氛围灯扩展 | `ESP32-C3_Bluetooth.ino`                              | 手机 BLE 指令      | WS2812 灯效             | 独立蓝牙扩展灯效控制 |
...
| bit3 | `SIG_RESERVED_USER`  | 预留用户交互事件 |
...
| 灯带数据引脚 | `GPIO5`               |
| 灯珠数量     | `61`                  |
| 左/右区域    | 左 `0~29`，右 `31~60` |

`LED 30` 在当前代码中未参与左右转向分区，可视为中间保留灯位。
...
onBleWrite(cmd)
{
    if (cmd == "brake") all_red();
    else if (cmd == "turnleft") enable_left_blink();
    else if (cmd == "turnright") enable_right_blink();
    else if (cmd == "danger") enable_danger_blink();
    else if (cmd == "offled") clear_all();
}
...
loop()
{
    ...
    leftLedBlink();
    DangerLedBlink();
    send_heartbeat_if_no_ack_message();
}
```
...
- 这是一个独立于 STM32 的子工程
- 当前没有 STM32 和 ESP32 之间的同步协议
- 代码中存在 `delay(1000)` 心跳节拍，因此实际闪烁频率会受到主循环节拍影响，更适合作为演示样例
...
ESP32 当前承担“人机交互演示”和“灯带效果验证”，未来若要并入主系统，建议新增：

1. STM32 与 ESP32 的 UART/CAN/BLE 桥接协议
2. 统一的灯效状态机
3. 车辆状态与氛围灯策略联动表
